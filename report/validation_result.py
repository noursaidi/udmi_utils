...
class ValidationResult:
    ...
    def __init__(self, device, schema):
        ...
        message_path = f"{os.path.normpath(device.site_path)}/out/devices/{device.device_id}/{schema}.json"
        error_path = f"{os.path.normpath(device.site_path)}/out/devices/{device.device_id}/{schema}.out"

        self.seen = os.path.isfile(message_path)
